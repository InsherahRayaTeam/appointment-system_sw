...
public class InMemoryUserRepository implements UserRepository {
    ...
    private void loadFromResource() {
        ...
        try (InputStream in = InMemoryUserRepository.class.getResourceAsStream(RESOURCE)) {
            if (in != null) {
                properties.load(in);
                seedUser(
                        properties.getProperty(ADMIN_ID_KEY, DEFAULT_ADMIN_ID),
                        properties.getProperty(ADMIN_EMAIL_KEY, DEFAULT_ADMIN_EMAIL),
                        resolveSecret(properties.getProperty(ADMIN_PASSWORD_PROPERTY), DEFAULT_ADMIN_PASSWORD_ENV),
                        UserRole.ADMIN
                );
                seedConfiguredRegularUsers(properties);
            }
        } catch (IOException e) {
        ...
        if (!usersByEmail.containsKey(DEFAULT_ADMIN_EMAIL)) {
            seedUser(
                    DEFAULT_ADMIN_ID,
                    DEFAULT_ADMIN_EMAIL,
                    resolveSecret(null, DEFAULT_ADMIN_PASSWORD_ENV),
                    UserRole.ADMIN
            );
        }
        ...
        if (!usersByEmail.containsKey(DEFAULT_USER_EMAIL)) {
            seedUser(
                    DEFAULT_USER_ID,
                    DEFAULT_USER_EMAIL,
                    resolveSecret(null, DEFAULT_USER_PASSWORD_ENV),
                    UserRole.USER
            );
        }
    }

    private String resolveSecret(String configuredSecret, String envKey) {
    ...
    private void seedConfiguredRegularUsers(Properties properties) {
        seedUser(
                properties.getProperty(USER_ID_KEY, DEFAULT_USER_ID),
                properties.getProperty(USER_EMAIL_KEY),
                resolveSecret(properties.getProperty(USER_PASSWORD_PROPERTY), DEFAULT_USER_PASSWORD_ENV),
                UserRole.USER
        );
        ...
        for (String propertyName : propertyNames) {
            ...
            seedUser(
                    properties.getProperty(userPrefix + ".id"),
                    properties.getProperty(userPrefix + ".email"),
                    resolveSecret(properties.getProperty(userPrefix + ".password"), envKey),
                    UserRole.USER
            );
        }
    }
